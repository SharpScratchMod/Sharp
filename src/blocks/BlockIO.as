...
public class BlockIO {
	...
	private static function blockToArray(b:Block):Array {
		...
		if (b.op == Specs.CALL){
			//result = [Specs.CALL, b.spec];			// procedure call - arguments follow spec
			result = [b.type = "b" ? Specs.CALL_BOOLEAN : b.type == "r" ? Specs.CALL_REPORTER : Specs.CALL, b.spec];
		}
	...
	private static function arrayToBlock(cmd:Array, undefinedBlockType:String, forStage:Boolean = false):Block {
		...
		if (cmd[0] == Specs.CALL || cmd[0] == Specs.CALL_BOOLEAN || cmd[0] == Specs.CALL_REPORTER) {
			//b = new Block(cmd[1], '', Specs.procedureColor, Specs.CALL);
			b = new Block(cmd[1], cmd[0] == Specs.CALL_BOOLEAN ? 'b' : cmd[0] == Specs.CALL_REPORTER ? 'r' : '', Specs.procedureColor, Specs.CALL);
			cmd.splice(0, 1);
		} else {
